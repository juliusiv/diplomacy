...
const Navigation = ({ active, ...props }) => {
  return (
    <nav
      className="text-gray-900 w-full bg-white flex flex-col items-center border-b border-yellow-900 shadow-sm"
      {...props}
    >
      <div className="flex flex-row w-8/12 items-baseline">
        ...
        {PageLinks.map(({ to, page, title }) => (
          <NavLink to={to} key={page}>
            {title}
          </NavLink>
        ))}
        <NavLink to="/login" key={Pages.Login} className="ml-auto">
          Login
        </NavLink>
      </div>
    </nav>
