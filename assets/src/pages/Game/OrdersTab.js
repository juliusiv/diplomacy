...
const Hold = ({ province }) => {
  return (
    <div>{province} hold</div>
  )
}

const Support = ({ province, support, to }) => {
  return (
    <div>{province} support {support} to {to}</div>
  )
}

const Move = ({ province, to }) => {
  return (
    <div>{province} move to {to}</div>
  )
}

const Convoy = ({ province, convoy, to }) => {
  return (
    <div>{province} convoy {convoy} to {to}</div>
  )
}
...
const OrdersTab = ({ orders, ...props }) => {
  return (
    <div>
      {orders.map(( order, i ) => <Order order={order} key={order.province} />)}
    </div>
  )
}
