...
const Row = ({
  ...
  return (
    <tr
      className={classNames("cursor-pointer hover:bg-red-200", className)}
...
const Header = ({ headers, ...props }) => {
  return (
    <thead className="border-b border-gray-900 bg-gray-400" {...props}>
      <tr>
        {headers.map((header) => (
          <th className="text-left pt-1 pb-1 pl-3 pr-3" key={header}>
            {header}
          </th>
        ))}
      </tr>
  ...
  return (
    <table
      className={classNames(
        "text-gray-900 shadow-sm border border-gray-900",
        className
      )}
      {...props}
    >
